...
public class SpnegoAuthenticatorTest {
            ...
            () -> {
              ...
              try {
                ...
                GSSName gssServerName =
                    manager.createName(BROKER, krb5PrincipalNameType, krb5Mechanism);
  ...
  public void testInexistentKeytabPath() throws Exception {
    ...
    try (SettingsOverride override = SettingsOverride.apply(Map.of(AppSettings.KEYTABS, config))) {
      try {
        ...
        assertEquals(
            "Keytab `/home/does-not-exist` in `authentication.spnego.keytabs` setting does not exist",
            e.getMessage());
      }
    }
  }
  ...
  public void testMissingValue() throws Exception {
    List<List<Map<String, String>>> configs =
        List.of(
    ...
    for (List<Map<String, String>> config : configs) {
      try (SettingsOverride override =
          SettingsOverride.apply(Map.of(AppSettings.KEYTABS, config))) {
        try {
          SpnegoAuthenticator auth = new SpnegoAuthenticator();
          auth.authenticateUser();
          fail();
          ...
          assertTrue(
              e.getMessage()
                  .startsWith(
                      "Invalid `authentication.spnego.keytabs` setting -- Error: hardcoded value: No configuration setting found for key"));
        }
      }
    }
  }
  ...
  public void testInvalidKeytab() throws Exception {
    ...
    List<Map<String, String>> config =
        List.of(Map.of("keytab", fakeKeytab.toString(), "principal", "blah"));
    try (SettingsOverride override = SettingsOverride.apply(Map.of(AppSettings.KEYTABS, config))) {
      try {
  ...
  public void testHeaderDoesntStartWithNegotiate() throws Exception {
    List<Map<String, String>> config =
        List.of(Map.of("keytab", fakeKDC.getKeytabPath(BROKER).toString(), "principal", BROKER));
    try (SettingsOverride override = SettingsOverride.apply(Map.of(AppSettings.KEYTABS, config))) {
      SpnegoAuthenticator auth = new SpnegoAuthenticator();
      try {
        ...
        assertEquals(Status.UNAUTHENTICATED.getCode(), e.getStatus().getCode());
        assertEquals(
            "UNAUTHENTICATED: Use \"authorization: Negotiate <token>\" metadata to authenticate",
            e.getMessage());
      }
    }
  }
  ...
  /**
   * Check the happy path: User generates a SPNEGO token, then the broker decrypts it to
