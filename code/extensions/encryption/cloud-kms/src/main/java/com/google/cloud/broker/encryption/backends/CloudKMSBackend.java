 ...
 * Cloud KMS is called once at startup to decrypt the static key. This is the preferred encryption
 * backend when high-throughput is a requirement.
 */
public class CloudKMSBackend extends AbstractEncryptionBackend {
  ...
  private static KeysetHandle readKeyset(String dekUri, String kekUri, CloudKMS kmsClient) {
    try {
      ...
      throw new RuntimeException(
          "Failed to read Keyset `" + dekUri + "` with KMS key `" + kekUri + "`", e);
    }
  }
  ...
  private static KeysetHandle generateAndWriteKeyset(
      KeyTemplate keyTemplate, String dekUri, String kekUri, CloudKMS kmsClient) {
    try {
      ...
      throw new RuntimeException(
          "Failed to write Keyset `" + dekUri + "` with KMS key `" + kekUri + "`", e);
    }
  }
