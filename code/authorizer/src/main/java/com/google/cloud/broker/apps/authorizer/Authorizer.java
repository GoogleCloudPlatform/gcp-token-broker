...
public class Authorizer implements AutoCloseable {
  ...
  private static final Credential.AccessMethod ACCESS_METHOD =
  ...
  private static final Set<String> SCOPES =
      ImmutableSet.of("https://www.googleapis.com/auth/devstorage.read_write", "email", "profile");
  ...
  private static final String host =
      AppSettings.getInstance().getString(AppSettings.AUTHORIZER_HOST);
  private static final int port = AppSettings.getInstance().getInt(AppSettings.AUTHORIZER_PORT);
  @VisibleForTesting AuthorizerServlet servlet;
  ...
  private static void initOauthFlow() {
    ...
    FLOW =
        new GoogleAuthorizationCodeFlow.Builder(HTTP_TRANSPORT, JSON_FACTORY, clientSecrets, SCOPES)
            ...
            .setApprovalPrompt(
                "force") // "offline" and "force" are both required to get a refresh token
            .build();
  }
  ...
  public static class AuthorizerServlet extends HttpServlet {
    ...
    void saveRefreshToken(String id, String value) {
      ...
      try {
        RefreshToken existingToken =
            (RefreshToken) AbstractDatabaseBackend.getInstance().get(RefreshToken.class, id);
    ...
    private void handleIndex(HttpServletRequest request, HttpServletResponse response)
      ...
      Map<String, Object> data =
          ImmutableMap.<String, Object>builder().put("GOOGLE_LOGIN_URI", GOOGLE_LOGIN_URI).build();
      String content =
          soySauce
    ...
    private void handleLoginRedirect(HttpServletRequest request, HttpServletResponse response)
        throws IOException {
    ...
    private void handleCallback(HttpServletRequest request, HttpServletResponse response)
      ...
      final TokenResponse tokenResponse =
          FLOW.newTokenRequest(authzCode)
              .setRedirectUri(((Request) request).getRootURL() + GOOGLE_OAUTH2_CALLBACK_URI)
              .execute();
      ...
      String content =
          soySauce.renderTemplate("Authorizer.Templates.success").renderHtml().get().getContent();
