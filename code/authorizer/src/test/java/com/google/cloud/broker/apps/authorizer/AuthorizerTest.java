...
public class AuthorizerTest {
  ...
  public static SettingsOverride settingsOverride =
      new SettingsOverride(
          Map.of(
              AppSettings.AUTHORIZER_HOST,
              "localhost",
              AppSettings.AUTHORIZER_PORT,
              String.valueOf(authorizerPort),
              AppSettings.OAUTH_CLIENT_ID,
              "FakeClientId",
              AppSettings.OAUTH_CLIENT_SECRET,
              "FakeClientSecret",
              AppSettings.ENCRYPTION_BACKEND,
              DummyEncryptionBackend.class.getCanonicalName(),
              AppSettings.DATABASE_BACKEND,
              DummyDatabaseBackend.class.getCanonicalName()));
  ...
  public void testGoogleRedirect() throws IOException {
    ...
    assertEquals(
        "https://accounts.google.com/o/oauth2/auth?access_type=offline&approval_prompt=force&client_id=FakeClientId&redirect_uri=http://localhost:"
            + authorizerPort
            + "/google/oauth2callback&response_type=code&scope=https://www.googleapis.com/auth/devstorage.read_write%20email%20profile",
  ...
  public void testRefreshTokenStore() throws DatabaseObjectNotFound {
    ...
    RefreshToken refreshToken =
        (RefreshToken)
            AbstractDatabaseBackend.getInstance().get(RefreshToken.class, "alice@example.com");

    assertNotNull(refreshToken);
    byte[] decrypted = AbstractEncryptionBackend.getInstance().decrypt(refreshToken.getValue());
