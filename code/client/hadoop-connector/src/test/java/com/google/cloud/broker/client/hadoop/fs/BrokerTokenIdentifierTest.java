...
public class BrokerTokenIdentifierTest {
  ...
  public String getSessionToken(Configuration conf) {
    ...
    BrokerTokenIdentifier identifier =
        new BrokerTokenIdentifier(conf, owner, renewer, null, service);
    ...
    public void getSessionToken(
      ...
      try {
        ...
        GetSessionTokenResponse response =
            GetSessionTokenResponse.newBuilder()
                .setSessionToken(
                    "FakeSessionToken/AuthenticatedUser="
                        + authenticatedUser
                        + ";Owner="
                        + request.getOwner()
                        + ";Target="
  ...
  public void testGetSessionTokenWithoutAccessBoundary() throws IOException {
    ...
    String token = getSessionToken(conf);
    assertEquals(
        "FakeSessionToken/AuthenticatedUser=" + ALICE + ";Owner=" + ALICE + ";Target=", token);
    UserGroupInformation.setLoginUser(null);
  }
}
