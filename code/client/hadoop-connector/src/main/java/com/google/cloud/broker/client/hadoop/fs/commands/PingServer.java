...
/**
 * This command runs several checks to verify that the Hadoop client is correctly configured to
 * access the broker server endpoints.
 *
 * <p>To run it:
 *
 * <p>java -cp $(hadoop classpath) com.google.cloud.broker.client.hadoop.fs.commands.PingServer
 */
public class PingServer {

  private static final String BUCKET = "gs://example";
  private static final Text SERVICE = new Text(BUCKET);
  private static final String CHECK_SUCCESS = "✅ Check successful\n";
  private static final String CHECK_FAIL = "\uD83D\uDED1 Check failed\n";
  ...
  private static void checkProviderDelegatedAuth(Configuration config, String sessionToken) {
    ...
    try {
      BrokerAccessTokenProvider provider =
          new BrokerAccessTokenProvider(SERVICE, CommandUtils.getBTI(sessionToken));
      provider.setConf(config);
      provider.refresh();
      assert provider.getAccessToken().getToken().startsWith("ya29.");
      System.out.println(CHECK_SUCCESS);
  ...
  private static String checkGetSessionToken(Configuration config) {
    try {
      ...
      BrokerTokenIdentifier identifier =
          new BrokerTokenIdentifier(config, username, username, username, SERVICE);
  ...
  private static void checkRenewSessionToken(Configuration config, String sessionToken)
      throws IOException {
    try {
  ...
  private static void checkCancelSessionToken(Configuration config, String sessionToken)
      throws IOException {
    try {
      Token<BrokerTokenIdentifier> token = CommandUtils.getTokenBTI(sessionToken, SERVICE);
      BrokerTokenRenewer renewer = new BrokerTokenRenewer();
