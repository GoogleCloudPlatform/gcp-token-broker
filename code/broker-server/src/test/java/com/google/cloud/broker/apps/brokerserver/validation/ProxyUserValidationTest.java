...
public class ProxyUserValidationTest {
  ...
  private static final Object proxyUsers =
  ...
  private static final Object userMappingRules =
      ConfigFactory.parseString(
              "rules=["
                  + "{"
                  + "if: \"true\","
      ...
      new SettingsOverride(
          Map.of(
              ...
              AppSettings.PROVIDER_BACKEND,
                  "com.google.cloud.broker.apps.brokerserver.accesstokens.providers.ServiceAccountProvider",
  ...
  public void testValidateImpersonatorInvalid() {
    ...
    for (String user : users) {
      try {
        ...
        assertEquals(
            String.format(
                "Impersonation of `%s` by `spark/testhost@EXAMPLE.COM` is not allowed", user),
  ...
  public void testValidateImpersonatorByUserAllowlist() {
    ...
    try {
      ...
      assertEquals(Status.PERMISSION_DENIED.getCode(), e.getStatus().getCode());
      assertEquals(
          "Impersonation of `charlie@EXAMPLE.COM` by `storm/testhost@EXAMPLE.COM` is not allowed",
          e.getStatus().getDescription());
    }
  }
  ...
  public void testValidateImpersonatorByGroupAllowlist() {
    ...
    for (String user : someUsers) {
      try {
        ...
        assertEquals(
            String.format(
                "Impersonation of `%s` by `hive/testhost@EXAMPLE.COM` is not allowed", user),
            e.getStatus().getDescription());
      }
    }
    ...
    for (String user : allUsers) {
      try {
        ...
        assertEquals(
            String.format(
                "Impersonation of `%s` by `solr/testhost@EXAMPLE.COM` is not allowed", user),
            e.getStatus().getDescription());
      }
    }
  }
