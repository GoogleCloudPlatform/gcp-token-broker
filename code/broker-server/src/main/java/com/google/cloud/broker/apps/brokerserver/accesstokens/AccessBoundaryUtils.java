...
public class AccessBoundaryUtils {
  ...
  public static AccessToken addAccessBoundary(AccessToken accessToken, String target) {
    ...
    List<String> permissions =
        AppSettings.getInstance().getStringList(AppSettings.ACCESS_TOKEN_BOUNDARY_PERMISSIONS);
    ...
    try {
      encodedAccessBoundary =
          URLEncoder.encode(gson.toJson(accessBoundary), StandardCharsets.UTF_8.toString());
    ...
    HttpPost httpPost =
        new HttpPost("https://securetoken.googleapis.com/v1alpha2/identitybindingtoken");
    ...
    try {
      ...
      accessTokenResponse =
          gson.fromJson(
              EntityUtils.toString(response.getEntity()), BoundedAccessTokenResponse.class);
      httpClient.close();
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
