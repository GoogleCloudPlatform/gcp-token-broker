...
public class CustomJsonLayout extends JsonLayout {
  ...
  protected Map toJsonMap(ILoggingEvent iLoggingEvent) {
    ...
    requestMetadata.put(
        "callerAddress", ClientAddressServerInterceptor.CLIENT_ADDRESS_CONTEXT_KEY.get());
    ...
    putIfExists(
        mdc,
        AbstractAuthenticationBackend.AUTHENTICATED_USER,
        authenticationInfo,
        "authenticatedUser");
    ...
    putIfExists(
        mdc, MDC_AUTH_MODE_PROXY_IMPERSONATED_USER_KEY, authenticationInfo, "impersonatedUser");
    ...
    for (Map.Entry<String, String> entry : mdc.entrySet()) {
      extras.put(StringUtils.removeStart(entry.getKey(), MDC_KEY_PREFIX), entry.getValue());
    }
